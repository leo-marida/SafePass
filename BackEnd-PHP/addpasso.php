...
try {
    ...
    // Validate input
    if (empty($user_id) || empty($user_key) || empty($account) || empty($username) || empty($password)) {
    ...
    $stmt = $conn->prepare('INSERT INTO passwords (user_id, account, username, password, iv, notes, created_at) VALUES (?, ?, ?, ?, ?, ?, NOW())');
    $stmt->execute([
        $user_id,
        $account,
        $username,
